(function () {
    ...
    query.prototype.isRemoved = function () {
        if (this.nodes[0]) {
            return this.nodes[0].parentNode === null;
        } else {
            return false;
        }
    };
    query.prototype.group = function () {
        if (!this.isEmpty()) {
            return this.nodes[0].datasets["-group-"] || null;
        } else {
            return null;
        }
    };
    ...
    plugin.prototype.delegate = function () {
        this.delegateEvent();
        this.delegateFind();
        return this;
    };
    ...
    brooder.Plugin = function (obj) {
        if (obj && obj.name && obj.name !== "") {
            var a = new Function("dom", "option", "this.__name__='" + obj.name + "';this.option=$.extend({},this.option,option);dom.data('" + obj.name + "',this);this.init&&this.init(dom,option);");
    ...
    var baseMapping = {
        ...
        compressPackets: [],
        compressTemplates: [],
        onimportcompressstart: null,
        onimportcompressend: null,
        onimportcompressprogress: null,
        _urlfilter: function () {
        },
        urlfilter: function (e) {
            var a = e.packet + ":" + e.type, b = baseMapping.filterMapping[a];
            if (b) {
                return b;
            } else {
                baseMapping._urlfilter && baseMapping._urlfilter(e);
            }
        },
    ...
    var _system = function (obj) {
        brooder.extend(baseMapping, obj);
    };
    _system.prototype.setCompressPackets = function (packets) {
        baseMapping.compressPackets = packets;
        return this;
    };
    _system.prototype.setCompressTemplates = function (templates) {
        baseMapping.compressTemplates = templates;
        return this;
    };
    _system.prototype.setCompressResourceImportListener = function (fn) {
        baseMapping.onimportcompressprogress = fn;
        return this;
    };
    _system.prototype.setCodeRewriter = function (fn) {
        if (fn) {
            baseMapping.docode = fn;
        }
        return this;
    };
    _system.prototype.setImportListener = function (packetName, callbacks) {
        if (arguments.length === 2) {
    ...
    _system.prototype.cleanImportListener = function () {
        baseMapping.onimportstart = null;
        baseMapping.onimportprogress = null;
        baseMapping.onimportend = null;
        return this;
    };
    _system.prototype.setPathFilter = function (fn) {
        if (is.isFunction(fn)) {
            baseMapping.urlfilter = fn;
        }
        return this;
    };
    _system.prototype.boot = function (selector, optionName) {
        var dom = null;
        if (arguments.length === 1) {
            dom = $("body");
            optionName = selector;
        ...
        packet.loadCompressResource(function () {
    ...
    brooder.System = function (obj) {
        return new _system(obj);
    };

    var packetSetting = function () {
        this.basePath = "";
        this.debug = false;
        this.compressCss = [];
        this.compressJs = [];
        this.compressPackets = [];
        this.compressTemplates = [];
        this.observers = {};
        this.filterMapping = {};
    ...
    packetSetting.prototype.filter = null;
    packetSetting.prototype.onloadstart = function (fn) {
        if (is.isFunction(fn)) {
            this._onloadstart = fn;
        }
    };
    packetSetting.prototype.onloadend = function (fn) {
        if (is.isFunction(fn)) {
            this._onloadend = fn;
        }
    };
    packetSetting.prototype.onloadprogress = function (fn) {
        if (is.isFunction(fn)) {
            this._onloadprogress = fn;
        }
    };
    packetSetting.prototype.urlfilter = function (e) {
        var a = e.packet + ":" + e.type, b = this.filterMapping[a];
        if (b) {
            return b;
        } else {
            var a = this.filter && this.filter(e);
            if (a !== undefined && a !== null) {
                return a;
            }
        }
    };
    packetSetting.prototype.setCodeRewriter = function (fn) {
        if (is.isFunction(fn)) {
            this._docode = fn;
        }
    };
    packetSetting.prototype.setCompressJs = function (mapping) {
        if (is.isArray(mapping)) {
            this.compressJs = mapping;
        }
    };
    packetSetting.prototype.setCompressCss = function (mapping) {
        if (is.isArray(mapping)) {
            this.compressCss = mapping;
        }
    };
    packetSetting.prototype.setCompressPackets = function (mapping) {
        if (is.isArray(mapping)) {
            this.compressPackets = mapping;
        }
    };
    packetSetting.prototype.setCompressTemplates = function (mapping) {
        if (is.isArray(mapping)) {
            this.compressTemplates = mapping;
        }
    };
    packetSetting.prototype.onimportstart = function (e) {
        this.observers[e.packet] && this.observers[e.packet].onimportstart && this.observers[e.packet].onimportstart(e);
    };
    packetSetting.prototype.onimportprogress = function (e) {
        this.observers[e.packet] && this.observers[e.packet].onimportprogress && this.observers[e.packet].onimportprogress(e);
    };
    packetSetting.prototype.onimportend = function (e) {
        this.observers[e.packet] && this.observers[e.packet].onimportend && this.observers[e.packet].onimportend(e);
    };
                ...
                this.load(ops.packetName, path, function () {
                    ...
                    if (re.length === ths.info.length) {
                        ...
                        for (var i = 0; i < ths.info.length; i++) {
                            ...
                            var xp = ths.info[i].code;
                            if (packet._docode) {
                                xp = packet._docode(ths.info[i].code, d);
                            }
                            var xcode = xp = packet.deleteR(xp) + "$.is.isEmptyObject(module.exports)?(module.exports=exports):'';";
                            if (d.usestrict === "true") {
                                d.usestrict = true;
    ...
    packet.getPacketInfo = function (str) {
        ...
        if (a && a.length > 0) {
            ...
            for (var o = 0; o < tp.length; o++) {
                ...
                var d = a.split(" ");
                if (d.length >= 2) {
                    var key = d[0].substring(1, d[0].length), value = d[1].substring(0, d[1].length - 1), suffix = null, compress = false;
                    if (!n[key]) {
                        n[key] = [];
                    }
                    ...
                    if (value.indexOf("(compress)") !== -1) {
                        compress = true;
                        value = value.replace(packet.g, "");
                    }
                    var t = value.split(":");
                    if (t.length > 1) {
                        n._packets_[t[t.length - 1]] = t[0];
                        if (key === "include" && suffix) {
                            n._packets_[t[t.length - 1]] = t[0] + "[" + suffix + "]";
                        }
                        ...
                        if (key === "include" && suffix) {
                            n._packets_[m[m.length - 1]] = t[0] + "[" + suffix + "]";
                        }
                    }
                    switch (key) {
                        case "packet":
                            n.packet = value;
                            n["path"] = basepath + n.packet.replace(packet.isdot, "/") + (suffix ? "." + suffix : ".js");
                            break;
                        case "require":
                            value = {
                                packet: value,
                            ...
                            n._depends.push(t[0]);
                            n.children.push(t[0]);
                            value.path = basepath + t[0].replace(packet.isdot, "/") + (suffix ? "." + suffix : ".js");
                            break;
                        case "include":
                            value = {
                                packet: value,
                        ...
                        case "json":
                            value = {
                                packet: value,
                                path: basepath + value.replace(packet.isdot, "/") + (suffix ? "." + suffix : ".json"),
                                value: null,
                                compress: compress
                            };
                            break;
                        case "template":
                            value = {
                                packet: value,
                                path: basepath + value.replace(packet.isdot, "/") + (suffix ? "." + suffix : ".html"),
                                value: null,
                                compress: compress
                            };
                            break;
                        ...
                        case "image":
                            value = {
                                packet: value,
                                path: basepath + value.replace(packet.isdot, "/") + (suffix ? "." + suffix : ".png"),
                                value: null,
                                compress: compress
                            };
                            break;
                        ...
                        case "css":
                            ...
                            if (value.packet.indexOf("http") === -1) {
                                if (value.packet.indexOf("/") === -1) {
                                    value.path = basepath + value.packet.replace(packet.isdot, "/") + (suffix ? "." + suffix : ".css");
                                } else {
                        ...
                        case "js":
                            ...
                            if (value.packet.indexOf("http") === -1) {
                                if (value.packet.indexOf("/") === -1) {
                                    value.path = basepath + value.packet.replace(packet.isdot, "/") + (suffix ? "." + suffix : ".js");
                                } else {
                                    value.path = basePath + value.packet;
                                }
                            }
    ...
    packet.precode = function (codetext) {
        var info = [], a = packet.isNoteall.exec(codetext);
        while (a) {
            var _packet = a[0].match(/@packet .*;/);
            if (_packet) {
                _packet = _packet[0].substring(8, _packet[0].length - 1);
                info.push({
                    index: a.index,
                    packet: _packet
                });
        ...
        for (var i = 0; i < info.length; i++) {
            ...
            packet.codemapping[info[i].packet] = codetext.substring(info[i].index, end);
        }
    };
    packet.loadCompressResource = function (fn) {
        if (baseMapping.compressCss.length + baseMapping.compressJs.length + baseMapping.compressPackets.length + baseMapping.compressTemplates.length > 0) {
            baseMapping.onimportcompressstart && baseMapping.onimportcompressstart();
            var queue = brooder.queue();
            queue.complete(function () {
                fn && fn();
                baseMapping.onimportcompressend && baseMapping.onimportcompressend();
            });
            queue.progress(function (a) {
                baseMapping.onimportcompressprogress && baseMapping.onimportcompressprogress(a);
            });
            for (var i = 0; i < baseMapping.compressCss.length; i++) {
                queue.add(function (a, b) {
                    loader.css(b, function () {
                        queue.next();
                    });
                }, null, baseMapping.compressCss[i]);
            }
            for (var i = 0; i < baseMapping.compressJs.length; i++) {
                queue.add(function (a, b) {
                    loader.js(b, function () {
                        queue.next();
                    });
                }, null, baseMapping.compressJs[i]);
            }
            for (var i = 0; i < baseMapping.compressPackets.length; i++) {
                queue.add(function (a, b) {
                    brooder.ajax({
                        url: b,
                        dataType: "text",
                ...
                }, null, baseMapping.compressPackets[i]);
            }
            for (var i = 0; i < baseMapping.compressTemplates.length; i++) {
                queue.add(function (a, b) {
                    brooder.ajax({
                        url: b,
                        dataType: "text",
                ...
                }, null, baseMapping.compressTemplates[i]);
            }
            queue.run();
        }else{
            fn && fn();
    ...
    packet.parseTemplate = function (template) {
        ...
        if (_a.length > 1) {
            ...
            for (var i = 0; i < _b.length; i++) {
                var pn = _b[i].substring(13, _b[i].length - 5);
                var temp = _a[i + 1];
                ...
                packet.templatemapping[pn] = _dommapping;
            }
        }
    };
    packet.load = function (e, fn, parent, pathname, iscompress) {
        var ths = this;
        ...
        for (var i = 0; i < aa.css.length; i++) {
            if (!(iscompress === true && aa.css[i].compress === true)) {
                var path = aa.css[i].path;
        ...
        for (var i = 0; i < aa.template.length; i++) {
            var path = aa.template[i].path;
//            console.log("----%o-----%o",aa.template[i].packet,packet.templatemapping[aa.template[i].packet]);
            if (!packet.templatemapping[aa.template[i].packet]) {
                path = baseMapping.urlfilter ? baseMapping.urlfilter({
                    type: "template",
                    path: path,
                ...
                queue.add(function (a, info) {
                    brooder.ajax({
                        ...
                        success: function (txt) {
                            packet.parseTemplate(txt);
                            var _dommapping = packet.templatemapping[info.packet];
                            if (_dommapping) {
                                info.value = _dommapping;
                                queue.next();
                            } else {
                                throw Error("[brooder] can not find template with packet name of " + info.packet + " in file of " + info.path);
                            }
                        }
                    });
                }, null, aa.template[i]);
            } else {
                aa.template[i].value = packet.templatemapping[aa.template[i].packet];
            }
        }
        ...
        for (var i = 0; i < aa.json.length; i++) {
            var path = aa.json[i].path;
            path = baseMapping.urlfilter ? baseMapping.urlfilter({
                type: "json",
                path: path,
                packet: aa.json[i].packet
            }) : path;
            if (!packet.jsonmapping[path]) {
                queue.add(function (a, b) {
                    brooder.ajax({
                        url: b.path,
                        type: "get",
                        ...
                        success: function (data) {
                            b.value = data;
                            packet.jsonmapping[path] = data;
                            queue.next();
                        }
                    });
                }, null, aa.json[i]);
            } else {
                aa.json[i].value = packet.jsonmapping[path];
            }
        }
        for (var i = 0; i < aa.html.length; i++) {
            var path = aa.html[i].path;
            path = baseMapping.urlfilter ? baseMapping.urlfilter({
                type: "html",
                path: path,
                packet: aa.html[i].packet
            }) : path;
            if (!packet.htmlmapping[path]) {
                queue.add(function (a, b) {
                    brooder.ajax({
                        url: b.path,
                        type: "get",
                        ...
                        success: function (data) {
                            b.value = data;
                            packet.htmlmapping[path] = data;
                            queue.next();
                        }
                    });
                }, null, aa.html[i]);
            } else {
                aa.html[i].value = packet.htmlmapping[path];
            }
        }
        for (var i = 0; i < aa.text.length; i++) {
            var path = aa.text[i].path;
            path = baseMapping.urlfilter ? baseMapping.urlfilter({
                type: "text",
                path: path,
                packet: aa.text[i].packet
            }) : path;
            if (!packet.textmapping[path]) {
                queue.add(function (a, b) {
                    brooder.ajax({
                        url: b.path,
                        type: "get",
                        ...
                        success: function (data) {
                            b.value = data;
                            packet.textmapping[path] = data;
                            queue.next();
                        }
                    });
                }, null, aa.text[i]);
            } else {
                aa.text[i].value = packet.textmapping[path];
            }
        }
        for (var i = 0; i < aa.image.length; i++) {
            ...
            if (!packet.imagemapping[path]) {
                ...
                aa.image[i].value = packet.imagemapping[path];
            }
        }
    ...
    packet.prototype.load = function (pkt, path, fn, parent) {
        ...
        if (packet.codemapping[pkt]) {
            packet.load.call(this, packet.codemapping[pkt], fn, parent, pathname, true);
        } else {
            path = baseMapping.urlfilter ? baseMapping.urlfilter({
                type: "js",
                path: path,
            ...
            brooder.ajax({
                ...
                type: "get",
                success: function (e) {
                    packet.load.call(ths, e, fn, parent, pathname, false);
                },
                error: function (e) {
                    fn && fn();
                }
    ...
    var _history = function (url) {
        ...
        router.add("404", function () {
            console.log("[brooder] router no page.")
        });
    ...
    var module = {
        ...
        get: function (target, moduleName, option, fn) {
            var p = moduleName.match(packet.issuffix), suffix = "";
            if (p) {
                suffix = p[0].substring(1, p[0].length - 1);
                moduleName = moduleName.replace(packet.issuffix, "");
            }
            if (!module.has(moduleName)) {
                var packetName = module.getPacketName(moduleName, suffix);
                brooder.packet({
    ...
    var option = {
        ...
        get: function (target, optionName, fn) {
            if (optionName && optionName !== "" && optionName !== "null") {
                var p = optionName.match(packet.issuffix), suffix = "";
                if (p) {
                    suffix = p[0].substring(1, p[0].length - 1);
                    optionName = optionName.replace(packet.issuffix, "");
                }
                if (optionName[0] !== "{" && optionName[0] !== "[") {
                    var a = option.has(optionName);
                    if (a === false) {
                        var packetName = module.getPacketName(optionName, suffix);
                        brooder.packet({
                            packetName: packetName,
                            target: target,
                            back: function () {
    ...
    group.prototype.each = function (fn) {
        if (fn) {
    ...
    group.prototype.get = function (groupname) {
        if (this._name.indexOf(groupname) !== -1) {
            var a = [];
            if (arguments.length === 1) {
                for (var i = 0; i < this._items.length; i++) {
                    if (this._items[i].data("-group-item-").groupId === groupname) {
                        a.push(this._items[i]);
                    }
                }
            }
            return new items(a);
    ...
    group.prototype.has = function (name) {
        return this._name.indexOf(name) !== -1;
    };
    ...
    items.prototype.get = function (key) {
        if (arguments.length === 0) {
            return this._items;
        } else if (arguments.length === 1) {
            if (is.isString(key)) {
                for (var i = 0; i < this._items.length; i++) {
                    if (this._items[i].data("-group-item-").itemId === key) {
    ...
    groups.prototype.clean = function () {
        this._items.length = 0;
    };
    groups.prototype.get = function (groupName) {
        if (is.isString(groupName)) {
            var r = [];
            for (var i = 0; i < this._items.length; i++) {
                var group = this._items[i].data("-group-");
    ...
    groups.prototype.each = function (fn) {
        if (fn) {
            for (var i = 0; i < this._items.length; i++) {
                var r = fn(this._items[i].data("-group-"), i);
                if (r === false) {
                    break;
                }
            }
    ...
    finders.prototype.clean = function () {
        this._items.length = 0;
    };
    finders.prototype.get = function (finderName, index) {
        var r = [];
        if (arguments.length === 1) {
            index = 0;
        }
        for (var i = 0; i < this._items.length; i++) {
            var name = this._items[i].data("-finder-");
            if (name === finderName) {
    ...
    finders.prototype.each = function (fn) {
        if (fn) {
            for (var i = 0; i < this._items.length; i++) {
                fn(this._items[i]);
            }
    ...
    module.add({
        ...
        _render: function (fn) {
            if (!this.dom.data("-view-")) {
                ...
                option.get(this.type(), optionName, function (ops) {
                    ...
                    if (ths.delegate) {
                        ths.dom.bind("DOMNodeInserted", function (e) {
                            var ant = $(this).data("-view-");
                            ant.privator("delegate", $(e.target));
                            ant.onnodeadd && ant.onnodeadd($(e.target));
                            e.stopPropagation();
                        }).bind("DOMNodeRemoved", function (e) {
                            if (!$(e.target).dataset("parentView")) {
                                var ant = $(this).data("-view-");
                                ant.privator("resetDomCache", $(e.target));
                                ant.onnoderemove && ant.onnoderemove($(e.target));
                            }
                            e.stopPropagation();
                        });
                    }
        ...
        _delegateEvent: function (dom) {
            var ths = this, domt = dom || this.dom, tp = domt.find("[data-bind]");
            domt.dataset("bind") && tp.add(domt);
            tp.length > 0 && console.log("event----" + ths.type() + "---" + ths.getId());
            tp.each(function () {
                if (!$(this).data("_eventback_")) {
        ...
        _delegateFind: function (dom) {
            var ths = this, domt = dom || this.dom, tp = domt.find("[data-find]");
            domt.dataset("find") && tp.add(domt);
            tp.length > 0 && console.log("find----" + ths.type() + "---" + ths.getId());
            tp.each(function () {
                if (!$(this).data("-finder-")) {
                    var name = $(this).dataset("find");
                    $(this).data("-finder-", name).removeAttr("data-find");
                    ths.finders._items.push($(this));
                    try {
                        ths["find_" + name] && ths["find_" + name]($(this), ths.finders);
                    } catch (e) {
                        console.error("[brooder] view finder called error with module of " + ths.type() + " Message:" + e.message);
                    }
                    $(this).attr("finder", name);
                }
            });
        },
        _delegateGroup: function (dom) {
            var ths = this, domt = dom || this.dom, tp = domt.find("*[data-group]");
            domt.dataset("group") && tp.add(domt);
            tp.length > 0 && console.log("group----" + ths.type() + "---" + ths.getId() + "----%o");
            tp.each(function () {
                var e = new group([], []);
                $(this).data("-group-", e).attr("group", $(this).dataset("group"));
                var namep = $(this).dataset("group").split(" ");
                for (var i = 0; i < namep.length; i++) {
                    e._name.push(namep[i]);
                }
                ths.groups._items.push($(this));
                $(this).find("*[data-groupi]").each(function () {
                    $(this).attr("groupi", $(this).dataset("groupi"));
                    var b = $(this).dataset("groupi").split(" ");
                    for (var i = 0; i < b.length; i++) {
                        var c = b[i].split("-");
                        e._items.push($(this).data("-group-item-", {
                            groupId: c[0],
                            itemId: c[1]
                        }));
                    }
                    $(this).removeAttr("data-groupi");
                });
                $(this).removeAttr("data-group");
                for (var i = 0; i < e._name.length; i++) {
                    if (ths["group_" + e._name[i]]) {
                        try {
                            ths["group_" + e._name[i]](e.get(e._name[i]), e);
                        } catch (e) {
                            console.error("[brooder] view groups called error with module of " + ths.type() + " Message:" + e.message);
                        }
                    }
                }
                $(this).bind("DOMNodeInserted", function (et) {
                    var pt = $(et.target), ptt = pt.find("[data-groupi]");
                    pt.dataset("groupi") && ptt.add(pt);
                    if (ptt.length > 0) {
                        console.log("----groupinner----add");
                        var tp = $(this).data("-group-").attr("group", $(this).dataset("group"));
                        tp && ptt.each(function () {
                            $(this).attr("groupi", $(this).dataset("groupi"));
                            var b = $(this).dataset("groupi").split(" ");
                            for (var i = 0; i < b.length; i++) {
                                var c = b[i].split("-");
                                tp._items.push($(this).data("-group-item-", {
                                    groupId: c[0],
                                    itemId: c[1]
                                }));
                            }
                ...
                }).bind("DOMNodeRemoved", function (et) {
                    var pt = $(et.target), ptt = pt.find(".incache");
                    pt.hasClass("incache") && ptt.add(pt);
                    if (ptt.length > 0) {
                        console.log("----groupinner----remove");
                        var grp = $(this).data("-group-"), dt = [];
                        ptt.each(function () {
                            if (!$(this).parentNode) {
                                dt.push($(this).get(0));
                            } else {
                        ...
                        var cp = [];
                        for (var i = 0; i < grp._items.length; i++) {
                            if (dt.indexOf(grp._items[i].get(0)) === -1) {
                                cp.push(grp._items[i]);
                            }
                        }
                        grp._items.length = 0;
                        grp._items = cp;
        ...
        _delegate: function (dom) {
            var domt = dom || this.dom;
            this.privator("delegateEvent", domt);
            this.privator("delegateGroup", domt);
            this.privator("delegateFind", domt);
            return this;
        },
        _resetDomCache: function (dom) {
            console.log("reset----" + this.type() + "----" + this.getId());
            var a = dom.find(".incache"), f = [], g = [], c = [], d = [];
            dom.hasClass("incache") && a.add(dom);
            a.each(function () {
            ...
            for (var i = 0; i < this.finders._items.length; i++) {
                if (f.indexOf(this.finders._items[i].get(0)) === -1) {
                    c.push(this.finders._items[i]);
                }
            }
            for (var i = 0; i < this.groups._items.length; i++) {
                if (g.indexOf(this.groups._items[i].get(0)) === -1) {
                    d.push(this.groups._items[i]);
                }
            }
            this.finders._items.length = 0;
            this.groups._items.length = 0;
            this.finders._items = c;
            this.groups._items = d;
        },
        getId: function () {
            ...
            if (!this.dom.data("-view-")) {
                ...
                option.get(this.type(), optionName, function (ops) {
                    ...
                    queue.complete(function (a) {
                        ...
                        if (a.delegate) {
                            a.dom.bind("DOMNodeInserted", function (e) {
                                if (!$(e.target).dataset("parentView")) {
                                    var ant = $(this).data("-view-");
                                    ant.privator("delegate", $(e.target));
                                    ant.onnodeadd && $(this).data("-view-").onnodeadd($(e.target));
                                }
                                e.stopPropagation();
                            }).bind("DOMNodeRemoved", function (e) {
                                if (!$(e.target).dataset("parentView")) {
        ...
        _addChild: function (xindex, option, callback) {
            ...
            if (ops.type && brooder.is.isString(ops.type) && ops.type !== "") {
                module.get(ths.type(), ops.type, null, function (sobj) {
                    ...
                    sobj.dom = brooder("<" + sobj.tagName + " class='" + cln + "' data-parent-view='" + ths.getId() + "' data-view='" + ops.type + "' data-view-id='" + ops.id + "' data-option='" + (is.isObject(ops.option) ? "" : ops.option) + "'></" + sobj.tagName + ">").appendTo(coner, false);
                    sobj.dom.bind("DOMNodeInserted", function (e) {
                        e.stopPropagation();
                    }).bind("DOMNodeRemoved", function (e) {
                        e.stopPropagation();
                    });
