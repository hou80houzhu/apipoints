...
Module({
    ...
    check: function () {
        var a = true, field = null, ps = $.promise(), qu = $.queue(), ths = this;
        qu.complete(function () {
            if (ths.customCheck) {
                a = ths.customCheck.call(ths);
            }
            if (!a) {
                ths.oncheckerror && ths.oncheckerror(field);
                ps.reject();
            } else {
                ps.resolve();
            }
        ...
        this.childEach(function () {
            ...
            if (this.typeOf("@.Aasyscheck")) {
                var fd = this;
                qu.add(function () {
                    var tq = this;
                    fd.asyscheck().done(function () {
                        tq.next();
                    }).fail(function () {
                        ps.reject();
                    });
    ...
    submit: function (fn) {
        ...
        this.check().done(function () {
            ths.getValues().done(function (data) {
...
Module({
    ...
    reg: {
        ...
        username: [/^[\u4E00-\u9FA5A-Za-z0-9-_]*$/, "中英文，数字，下划线，减号"],
    ...
    check: function () {
        ...
        if (!this.customCheck) {
            ...
            if (!result) {
                ...
                this.showTip("长度应该大于等于" + this.option.min + "小于等于" + this.option.max);
                return false;
            } else {
                this.hideTip();
            }
            ...
            if (!result) {
                this.showTip(this.reg[this.option.reg][1] || this.option.regMes);
                return false;
            } else {
                this.hideTip();
            }
    ...
    getValue: function () {
        this.value = this.input.val()!==null?this.input.val().trim():'';
        return  this.value;
    },
    disable: function (isdisable) {
        if (isdisable) {
...
Module({
    ...
    extend: "@.field",
    option:{},
    template:module.getTemplate("@formtemplate","edittext"),
    init:function(){
        this.render(this.option);
    },
    find_area:function(dom){
        var ths=this;
        this.area=dom;
        dom.click(function(){
            ths.dispatchEvent("editarea",{
                val:$(this).html()==="点击编辑"?"":$(this).html(),
                btn:$(this)
            });
        });
    },
...
Module({
    name:"divtext",
    extend:"@.field",
    template:module.getTemplate("@formtemplate","divtext"),
    init:function(){
        this.render(this.option);
    },
    setValue:function(a){
        this.value=a;
        this.dom.find(".divtext").html(a);
        return this;
    },
    getValue:function(){
        return this.dom.find(".divtext").html();
    }
});
Module({
    name: "statictext",
...
Module({
    ...
    btn_checkbtn: function () {
        ...
        this.getLastChild().getValues().done(function (data) {
            ...
            if (data) {
                ...
                ths.getLastChild().submit().done(function (t) {
                    $.extend(p,t);
    ...
    event_treelite_click: function (e) {
        if (e.data.level != "1") {
            ...
            this.dispatchEvent("treelite_click_level",{level:e.data.level});
        }
    },
    event_treelite_add: function (e) {
        ...
        this.dispatchEvent("treelite_add_level",{level:e.data.level});
    },
...
Module({
    ...
    asyscheck: function () {
        var t = {}, ps = $.promise();
        t[this.option.checkPropName] = this.getValue();
        this.postRequest(this.option.url, t).data(function () {
            this.hideTip();
            ps.resolve();
        }).bad(function (a) {
            this.showTip(a.msg ? a.msg : this.option.errorTip);
            ps.reject();
        }).error(function () {
            this.showTip(this.option.errorTip);
            ps.reject();
        });
        return ps;
    }
...
Module({
    ...
    add:function(option){
        var a=$("<div></div>").appendTo(this.dom.find(".listform-container"));
    ...
    hide:function(nums){
        var a=this.dom.find(".listform-container").children();
        for(var i in nums){
            a.eq(nums[i]).hide();
        }
    },
...
Module({
    ...
    init: function () {
        if (this.option.width) {
            this.dom.width(this.option.width).addClass("wrap");
        }
    },
    btn_submit: function () {
        this.submit();
...
Module({
    ...
    check: function () {
        if (this.option.required && this.getFileSize() === 0 && this._files.children().length === 0) {
            this.showTip("请选择文件");
            return false;
        }
        return true;
